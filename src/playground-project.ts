...
export class PlaygroundProject extends LitElement {
  ...
  private async _loadProjectFromSource() {
    ...
    switch (source.type) {
      ...
      default: {
        ...
        throw new Error(
          `Unexpected project source: ${JSON.stringify(exhaustiveCheck)}`,
        );
      }
    }
    this._pristineFiles =
  ...
  moveFileAfter(firstIndex: number, secondIndex: number) {
    if (
      ...
      firstIndex < 0 ||
      secondIndex < 0 ||
      firstIndex >= this._files.length ||
      secondIndex >= this._files.length ||
    ...
    this._files.splice(
      secondIndex + (firstIndex < secondIndex ? 0 : 1),
      0,
      file,
    );
